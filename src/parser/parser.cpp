...
QueryInfo SyntaxValidator::validateAndExtract(const string& query) {
    ...
    static const regex selectPattern(
        R"(SELECT\s+((\*)|(\w+)|([\w\s,]+))\s+FROM\s+(\w+)(?:\s+WHERE\s+(.+))?)",
    ...
    static const regex createIndexPattern(
        R"(CREATE\s+INDEX\s+(\w+)\s+ON\s+(\w+)\s*\((\w+)\))",
        regex_constants::icase);
    ...
    static const regex insertPattern(
    ...
    static const regex deletePattern(
    ...
    static const regex updateWithWherePattern(
        R"(UPDATE\s+(\w+)\s+SET\s+([\w\s=',.]+)\s+WHERE\s+(.+))",
        regex_constants::icase);
    if (regex_match(query, matches, updateWithWherePattern)) {
        info.type = "UPDATE";
        info.tableName = matches[1].str();
        info.columns = splitAndTrim(matches[2].str());
    ...
    static const regex updatePattern(
    ...
    static const regex createTablePattern(
        R"(CREATE\s+TABLE\s+(\w+)\s*\(([\w\s,]+)\))",
        regex_constants::icase);
...
vector<ExecutionStep> ExecutionPlanGenerator::generatePlan(const QueryInfo& queryInfo) {
    ...
    if (queryInfo.type == "SELECT") {
        ...
        if (!queryInfo.condition.empty()) {
            plan.push_back({"Filter", queryInfo.condition, "Applying WHERE clause"});
        }
        plan.push_back({"Projection", join(queryInfo.columns), "Selecting columns"});
        ...
        plan.push_back({"Create Table", queryInfo.tableName, "Creating new table"});
    }
...
vector<ExecutionStep> QueryAnalyzer::analyze(const string& query) {
    ...
    return planGenerator.generatePlan(queryInfo);
}
